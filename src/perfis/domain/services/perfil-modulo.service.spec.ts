...
import { ModuloNaoEncontradoErro, PerfilModuloJaCadastradoErro, PerfilNaoEncontradoErro } from "../erros";
...
describe('PerfilModuloService', () => {
  ...
  describe('adicionaPerfilModulo', () => {
    it('deve jogar um erro caso perfil não exista', async () => {
      await expect(service.adicionaPerfilModulo(1, 1, 1)).rejects.toThrowError(PerfilNaoEncontradoErro);
    });
    ...
    it('deve jogar um erro caso modulo não exista', async () => {
      ...
      await expect(service.adicionaPerfilModulo(usuario.id, perfil.id, 1)).rejects.toThrowError(ModuloNaoEncontradoErro);
    });
    ...
    it('deve adicionar um novo modulo ao perfil', async () => {
      ...
      const perfilModulo  =await service.adicionaPerfilModulo(usuario.id, perfil.id, modulo.id);
    ...
    it('deve jogar um erro ao tentar adicionar um modulo ao perfil duas vezes', async () => {
      ...
      await expect(service.adicionaPerfilModulo(usuario.id, perfil.id, modulo.id)).rejects.toThrowError(PerfilModuloJaCadastradoErro)
    });
    ...
    it('deve jogar um erro ao tentar adicionar um modulo ao perfil duas vezes', async () => {
      ...
      const perfil = await perfilService.cadastrar(usuario.id, perfilFactory({}));
      const modulo = await moduloService.cadastrar(usuario.id, moduloFactory({}));
      await expect(service.adicionaPerfilModulo(100, perfil.id, modulo.id)).rejects.toThrowError(UsuarioNaoEncontrado)
    });
  })
});