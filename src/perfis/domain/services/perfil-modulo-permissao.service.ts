...
export class PerfilModuloPermissaoService {
  ...
  async cadastrar(usuarioId: number, perfilModuloId: number, permissaoId: number): Promise<PerfilModuloPermissao> {
    const perfilModulo = await this.perfilModuloService.buscaPorId(perfilModuloId);
    const permissao = await this.permissaoService.buscaPorId(permissaoId);

    const perfilModuloPermissaoEncontrado = await this.perfilModuloPermissaoCadastrado(perfilModulo, permissao);
    ...
    const perfilModuloPermissao = new PerfilModuloPermissao({
      perfilModulo,permissao
    })

    const perfilModuloPermissaoRastreavel = await this.rastreamentoService.adicionaRastreioInclusao(usuarioId, perfilModuloPermissao)

    return this.perfilModuloPermissaoRepository.save(perfilModuloPermissaoRastreavel);
  }

  private async perfilModuloPermissaoCadastrado(perfilModulo: PerfilModulo, permissao: Permissao){
    return this.perfilModuloPermissaoRepository.findOne({ where: { perfilModulo, permissao } });
  }
}