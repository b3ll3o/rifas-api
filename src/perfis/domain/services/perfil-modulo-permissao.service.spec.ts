...
import {
  ...
  PerfilModuloPermissaoJaCadastradoErro,
...
describe('PerfilModuloPermissaoService', () => {
  ...
  describe('cadastrar', () => {
    it('deve jogar um erro caso perfil modulo não exista', async () => {
      ...
      await perfilService.cadastrar(usuario.id, perfilFactory({}));
      await moduloService.cadastrar(usuario.id, moduloFactory({}));
      const permissao = await permissaoService.cadastrar(
        usuario.id,
        permissaoFactory({}),
      );
      await expect(
        service.cadastrar(usuario.id, 100, permissao.id),
      ).rejects.toThrowError(PerfilModuloNaoEncontradoErro);
    });
    ...
    it('deve jogar um erro caso permissão não exista', async () => {
      ...
      const perfilModulo = await perfilModuloService.adicionaPerfilModulo(
        usuario.id,
        perfil.id,
        modulo.id,
      );
      await expect(
        service.cadastrar(usuario.id, perfilModulo.id, 100),
      ).rejects.toThrowError(PermissaoNaoEncontradoErro);
    });
    ...
    it('deve retorna um perfil modulo permissao', async () => {
      ...
      const perfilModuloPermissao = await service.cadastrar(
        usuario.id,
        perfilModulo.id,
        permissao.id,
      );
    ...
    it('deve jogar um erro quando tentar adicionar uma permissão ao perfil modulo mais de uma vez', async () => {
      ...
      await expect(
        service.cadastrar(usuario.id, perfilModulo.id, permissao.id),
      ).rejects.toThrowError(PerfilModuloPermissaoJaCadastradoErro);
    });
  });
});
