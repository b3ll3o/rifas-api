...
export class PerfilModuloService {
  constructor(@InjectRepository(PerfilModulo)
  ...
  async adicionaPerfilModulo(usuarioId: number, perfilId: number, moduloId: number): Promise<PerfilModulo> {
    const perfil = await this.perfilService.buscaPorId(perfilId);
    const modulo = await this.moduloService.buscaPorId(moduloId);

    const perfilModuloEncontrado = await this.perfilModuloCadastrado(perfil, modulo)
    ...
    const perfilModuloRastreavel = await this.rastreamentoService.adicionaRastreioInclusao(usuarioId, perfilModulo);

    return this.perfilModuloRepository.save(perfilModuloRastreavel);
  }

  private async perfilModuloCadastrado(perfil: Perfil, modulo: Modulo): Promise<PerfilModulo> {
